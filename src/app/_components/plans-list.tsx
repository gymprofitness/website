...
// Memoized plan card component with image at absolute top
const PlanCard = memo(({ 
  plan, 
  isPopular, 
  price, 
  ...
  <div className={`group relative flex flex-col h-full rounded-xl overflow-hidden transition-all duration-500 will-change-transform ${
    isPopular 
    ...
    {/* Plan image at the absolute top - no padding/margin whatsoever */}
    <div className="relative h-40 sm:h-48 w-full overflow-hidden">
      {plan.images && plan.images.length > 0 ? (
        ...
        <div className={`h-full w-full flex items-center justify-center ${
          ...
          <Zap className={`h-16 w-16 ${isPopular ? "text-orange-400" : "text-gray-500"}`} />
        </div>
      )}
      ...
      <div className="absolute bottom-4 left-4 right-4">
        <h3 className="text-xl sm:text-2xl font-bold text-white mb-1 text-left">{plan.name}</h3>
        {isPopular && (
...
const PlansLoadingSkeleton = memo(() => (
  <div className="space-y-8">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {[...Array(3)].map((_, i) => (
        <div key={i} className="h-96 bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl animate-pulse border border-gray-700" />
      ))}
    </div>
  </div>
));
...
function PlansList() {
  ...
  // Memoized billing cycles with discounts
  ...
  const getPriceByBillingCycle = useCallback((plan: IPlan) => {
    ...
    <div className="space-y-8 sm:space-y-12">
      ...
      <div className="flex flex-col items-center justify-center space-y-6">
        <div className="text-center space-y-2">
          <h2 className="text-xl sm:text-2xl font-bold text-white">Choose Your Billing Cycle</h2>
          <p className="text-sm sm:text-base text-gray-400">Save more with longer commitments</p>
        </div>
      ...
      {plans.length === 0 ? (
        <Card className="bg-gradient-to-br from-gray-900 to-gray-800 border-gray-700/50">
          <CardContent className="text-center py-12">
            ...
            <p className="text-gray-400">Check back later for exciting new plans!</p>
            <Button 
              onClick={fetchPlans}
              variant="outline"
        ...
        <>
          ...
          <div className="text-center space-y-4 pt-8 border-t border-gray-800/50">
            ...
            <p className="text-xs text-gray-500 max-w-2xl mx-auto">
              ...
              Prices shown are in Indian Rupees (₹) and exclude applicable taxes.
            </p>
          </div>
        </>
      )}
